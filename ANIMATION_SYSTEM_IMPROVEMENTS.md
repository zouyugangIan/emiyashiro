...
### 背景系統
遊戲使用**程序化雲彩系統**（`src/systems/background.rs`）作為遊戲背景：
- ✅ 雲彩每 5 秒自動生成
- ✅ 雲彩從右向左移動
- ✅ 雲彩由 3 個白色圓圈組成
- ✅ 雲彩在離開屏幕後自動清理
...
### 角色動畫 vs 背景
- **角色動畫**：用於玩家控制的精靈（Shirou 和 Sakura）
- **背景系統**：使用程序化雲彩，不使用角色圖片
- **主菜單背景**：使用封面圖片輪播（`menu.rs` 中的 `CoverImage1` 和 `CoverImage2`）

`setup_animated_background` 函數已標記為備用，目前不使用。
