...
- 结构：12 段连续浮岛主线、31 组高空平台、一条风轮机井纵向支路，以及 11 组坑洞恢复路线。
...
- `CombatGate(arena)`
- `Goal`
- `ClimbAnchor(direction)`：只在设计好的坑洞恢复线上启用；方向指向可上翻的平台。
...
每个主线缺口保留上层坑口轮廓，同时在下方 160 px 设置云台，并以 96 px 高差的交错踏板接回主路。
全图共有 33 个显式抓边点，使用青色横条与金色垂标提示；下落靠近标记时自动抓住，`W/Space`
上翻，`S/↓` 松手。普通墙面没有抓边语义，避免高速战斗中误吸附。
...
- 11 个主线坑洞均具备“救援云台 → 中段踏板 → 抓边上翻”的非致死恢复路线。
