...
- 验收：
  - [ ] 断线后 5 秒内重连可自动回到 Playing 流程。
  - [ ] `MyNetworkId` 与实体映射恢复，不产生重复实体。
  - [ ] `NetworkStatus` 全程与真实连接事件一致。
...
- 优先级：P1
- 验收：
  - [ ] `WorldSnapshot` 支持 delta 编码。
  - [ ] 大量实体场景下网络负载下降（记录压测数据）。
