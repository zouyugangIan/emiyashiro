...
> 文件性質：功能說明（非版本發布說明）  
> 最後與代碼對齊：2026-02-23（對齊文件：`src/systems/scene_decoration.rs`、`src/plugins/presentation.rs`、`src/plugins/ui.rs`）
...
- [x] `setup_parallax_background` 在 `OnEnter(GameState::Playing)` 註冊
- [x] 雲彩（3 秒）與地面裝飾（2 秒）生成頻率符合實作
- [x] 遠景循環與離屏清理策略存在且已分層處理
- [x] `dynamic_lighting` 僅作用於 FarBackground 層
- [x] 返回 `GameState::Menu` 時會清理場景裝飾實體
- [ ] 效能壓測基準（不同機型 FPS 影響）尚未文檔化
