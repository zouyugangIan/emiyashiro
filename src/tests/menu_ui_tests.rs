...
mod property_tests {
    ...
    fn test_cover_images_use_ui_node_components() {
        ...
        let mut cover1_query = app.world_mut().query_filtered::<Entity, With<CoverImage1>>();
        let cover1_entities: Vec<Entity> = cover1_query.iter(app.world()).collect();
        
        // Query for CoverImage2 entities
        let mut cover2_query = app.world_mut().query_filtered::<Entity, With<CoverImage2>>();
        let cover2_entities: Vec<Entity> = cover2_query.iter(app.world()).collect();
        
        let all_cover_entities: Vec<Entity> = cover1_entities.into_iter()
        ...
        for entity in all_cover_entities {
            ...
            assert!(has_node, "CoverImage entity {:?} should have Node component", entity);
            assert!(!has_sprite, "CoverImage entity {:?} should NOT have Sprite component", entity);
        }
    }
    ...
    fn test_cover_images_with_assets_use_ui_nodes() {
        ...
        assert!(!cover1_entities.is_empty(), "Should have CoverImage1 entities");
        assert!(!cover2_entities.is_empty(), "Should have CoverImage2 entities");
        ...
        for entity in all_cover_entities {
            ...
            assert!(has_node, "CoverImage entity {:?} should have Node component", entity);
            assert!(has_image_node, "CoverImage entity {:?} should have ImageNode component", entity);
            assert!(!has_sprite, "CoverImage entity {:?} should NOT have Sprite component", entity);
        }
    }
    ...
    fn test_cover_images_fill_entire_window() {
        ...
        let mut cover1_query = app.world_mut().query_filtered::<(Entity, &Node), With<CoverImage1>>();
        let cover1_entities: Vec<(Entity, Node)> = cover1_query.iter(app.world())
            .map(|(e, n)| (e, n.clone()))
            .collect();
        
        // Query for CoverImage2 entities with Node component
        let mut cover2_query = app.world_mut().query_filtered::<(Entity, &Node), With<CoverImage2>>();
        let cover2_entities: Vec<(Entity, Node)> = cover2_query.iter(app.world())
            .map(|(e, n)| (e, n.clone()))
            .collect();
        
        let all_cover_entities: Vec<(Entity, Node)> = cover1_entities.into_iter()
            .chain(cover2_entities.into_iter())
            .collect();
        
        assert!(!all_cover_entities.is_empty(), "Should have cover image entities");
    ...
    fn test_z_axis_layering_correct() {
        ...
        let mut cover1_query = app.world_mut().query_filtered::<&ZIndex, With<CoverImage1>>();
        let cover1_zindices: Vec<ZIndex> = cover1_query.iter(app.world())
            .map(|z| *z)
            .collect();
        
        // Query for CoverImage2 entities with ZIndex component
        let mut cover2_query = app.world_mut().query_filtered::<&ZIndex, With<CoverImage2>>();
        let cover2_zindices: Vec<ZIndex> = cover2_query.iter(app.world())
            .map(|z| *z)
            .collect();
        
        assert!(!cover1_zindices.is_empty(), "Should have CoverImage1 entities with ZIndex");
        assert!(!cover2_zindices.is_empty(), "Should have CoverImage2 entities with ZIndex");
        
        // Verify that CoverImage2 has higher ZIndex than CoverImage1
        for cover1_z in &cover1_zindices {
            for cover2_z in &cover2_zindices {
                assert!(cover2_z.0 > cover1_z.0, 
...
mod button_interaction_tests {
    ...
    fn test_button_interactions_update_background_color() {
        ...
        let mut start_button_query = app.world_mut().query_filtered::<(Entity, &BackgroundColor), With<StartButton>>();
        let start_buttons: Vec<(Entity, BackgroundColor)> = start_button_query.iter(app.world())
            .map(|(e, bg)| (e, bg.clone()))
            .collect();
        
        assert!(!start_buttons.is_empty(), "Should have StartButton entities");
        ...
        for (entity, bg_color) in &start_buttons {
            ...
            assert!(app.world().entity(*entity).contains::<BackgroundColor>(),
                "StartButton entity {:?} should have BackgroundColor component", entity);
            
            // Verify initial color is set (default state)
            let color = bg_color.0;
            assert!(color.alpha() > 0.0, 
                "StartButton BackgroundColor should have non-zero alpha, got {}", color.alpha());
        }
        ...
        let mut start_button_query = app.world_mut().query_filtered::<&BackgroundColor, With<StartButton>>();
        let hovered_colors: Vec<BackgroundColor> = start_button_query.iter(app.world())
            .map(|bg| bg.clone())
            .collect();
        ...
        for bg_color in &hovered_colors {
            ...
            assert!((srgba.red - 0.3).abs() < 0.01, 
                "Hovered button red channel should be ~0.3, got {}", srgba.red);
            assert!((srgba.alpha - 0.8).abs() < 0.01,
                "Hovered button alpha should be ~0.8, got {}", srgba.alpha);
        }
    }
    ...
    fn test_start_button_triggers_playing_state() {
        ...
        assert_eq!(**current_state, GameState::Menu, "Initial state should be Menu");
        
        // Find and press the start button
        let mut start_button_query = app.world_mut().query_filtered::<Entity, With<StartButton>>();
        ...
        assert_eq!(**new_state, GameState::Playing, "State should transition to Playing after start button press");
    }
    ...
    fn test_load_button_triggers_load_table_state() {
        ...
        let mut load_button_query = app.world_mut().query_filtered::<Entity, With<crate::systems::ui::LoadButton>>();
        ...
        let new_state = app.world().resource::<State<GameState>>();
        assert_eq!(**new_state, GameState::LoadTable, "State should transition to LoadTable after load button press");
    }
    ...
    fn test_character_select_button_updates_selection() {
        ...
        let mut char_button_query = app.world_mut().query_filtered::<(Entity, &CharacterSelectButton), ()>();
        let char_buttons: Vec<(Entity, CharacterType)> = char_button_query.iter(app.world())
            .map(|(e, btn)| (e, btn.character_type.clone()))
            .collect();
        
        assert!(!char_buttons.is_empty(), "Should have CharacterSelectButton entities");
        ...
        for (entity, char_type) in &char_buttons {
            if *char_type != initial_char {
                ...
                assert_eq!(new_selection.selected_character, *char_type,
                    "Character selection should update to {:?}", char_type);
...
mod responsive_layout_tests {
    ...
    fn test_ui_elements_use_percentage_based_sizing() {
        ...
        let mut cover_query = app.world_mut().query_filtered::<&Node, Or<(With<CoverImage1>, With<CoverImage2>)>>();
        let cover_nodes: Vec<Node> = cover_query.iter(app.world())
            .map(|n| n.clone())
            .collect();
        ...
        for node in &cover_nodes {
            match node.width {
                ...
                _ => panic!("Cover node width should use Val::Percent, got {:?}", node.width),
            }
            ...
            match node.height {
                ...
                _ => panic!("Cover node height should use Val::Percent, got {:?}", node.height),
            }
        }
        
        // Query for the main UI root node - should also use percentage-based sizing
        let mut ui_root_query = app.world_mut().query_filtered::<&Node, (With<MenuUI>, Without<CoverImage1>, Without<CoverImage2>)>();
        let ui_root_nodes: Vec<Node> = ui_root_query.iter(app.world())
            .map(|n| n.clone())
            .collect();
        
        // Find nodes that are likely the main UI container (100% width/height)
        let full_size_nodes: Vec<&Node> = ui_root_nodes.iter()
            .filter(|n| matches!(n.width, Val::Percent(100.0)) && matches!(n.height, Val::Percent(100.0)))
            .collect();
        
        assert!(!full_size_nodes.is_empty(), 
        ...
        for node in full_size_nodes {
            assert!(matches!(node.width, Val::Percent(_)), 
                "UI root node should use percentage-based width");
            assert!(matches!(node.height, Val::Percent(_)), 
...
mod cleanup_tests {
    ...
    fn test_cleanup_removes_all_menu_entities() {
        ...
        assert!(!menu_entities_before.is_empty(), 
            "Should have MenuUI entities before cleanup, found {}", 
            menu_entities_before.len());
        
        println!("MenuUI entities before cleanup: {}", menu_entities_before.len());
        ...
        println!("MenuUI entities after cleanup: {}", menu_entities_after.len());
        
        // Verify all MenuUI entities were removed
        assert!(menu_entities_after.is_empty(), 
        ...
        let mut cover1_query = app.world_mut().query_filtered::<Entity, With<CoverImage1>>();
        let cover1_entities: Vec<Entity> = cover1_query.iter(app.world()).collect();
        assert!(cover1_entities.is_empty(), 
            "CoverImage1 entities should be removed after cleanup");
        
        let mut cover2_query = app.world_mut().query_filtered::<Entity, With<CoverImage2>>();
        let cover2_entities: Vec<Entity> = cover2_query.iter(app.world()).collect();
        assert!(cover2_entities.is_empty(), 
        ...
        assert!(start_button_entities.is_empty(), 
...
mod integration_tests {
    ...
    fn test_complete_menu_lifecycle() {
        ...
        let mut menu_query = app.world_mut().query_filtered::<Entity, With<MenuUI>>();
        let menu_entities: Vec<Entity> = menu_query.iter(app.world()).collect();
        assert!(!menu_entities.is_empty(), "Menu should be initialized with entities");
        ...
        let mut menu_query_after = app.world_mut().query_filtered::<Entity, With<MenuUI>>();
        let menu_entities_after: Vec<Entity> = menu_query_after.iter(app.world()).collect();
        assert!(menu_entities_after.is_empty(), "All menu entities should be cleaned up");
    }
    ...
    fn test_animation_system_integration() {
        ...
        let initial_alphas: Vec<(f32, f32)> = cover_query.iter(app.world())
        ...
        let mut cover_query_after = app.world_mut().query_filtered::<&BackgroundColor, Or<(With<CoverImage1>, With<CoverImage2>)>>();
        let final_alphas: Vec<f32> = cover_query_after.iter(app.world())
            .map(|bg| bg.0.alpha())
            .collect();
        ...
        for alpha in final_alphas {
            assert!(alpha >= 0.0 && alpha <= 1.0, 
                "Alpha should be in valid range [0, 1], got {}", alpha);
        }
    }
    ...
    fn test_menu_fallback_without_assets() {
        ...
        let mut fallback_query = app.world_mut().query_filtered::<Entity, (With<MenuUI>, Without<CoverImage1>, Without<CoverImage2>)>();
