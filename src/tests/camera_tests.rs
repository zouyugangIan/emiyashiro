...
mod tests {
    ...
    fn test_movement_speed_limit() {
        ...
        let clamped_movement = large_movement.clamp(-max_movement_per_frame, max_movement_per_frame);
        assert_eq!(clamped_movement, max_movement_per_frame);
        
        // 测试小距离移动不被限制
        let small_movement = 2.0f32;
        let clamped_small_movement = small_movement.clamp(-max_movement_per_frame, max_movement_per_frame);
