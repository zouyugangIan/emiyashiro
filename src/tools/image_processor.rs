...
impl ImageProcessor {
    /// 创建简单的角色精灵
    pub fn create_character_sprite(width: u32, height: u32, character_type: &str) -> Image {
    ...
    pub fn create_animation_frames(
        ...
        for i in 0..frame_count {
            ...
            if let Some(data) = image.data.as_mut() {
                let offset = ((i as f32 * std::f32::consts::PI * 2.0 / frame_count as f32).sin()
                    * 2.0) as i32;
...
/// 程序化生成角色精灵系统
pub fn generate_character_sprites(_commands: Commands, mut images: ResMut<Assets<Image>>) {
    println!("🎨 生成程序化角色精灵...");

    // 生成士郎精灵
    let shirou_sprite = ImageProcessor::create_character_sprite(64, 96, "shirou");
    let _shirou_handle = images.add(shirou_sprite);

    // 生成樱精灵
    let sakura_sprite = ImageProcessor::create_character_sprite(64, 96, "sakura");
    let _sakura_handle = images.add(sakura_sprite);

    // 生成动画帧
    let _shirou_frames = ImageProcessor::create_animation_frames("shirou", 4, 64, 96);
    let _sakura_frames = ImageProcessor::create_animation_frames("sakura", 4, 64, 96);
