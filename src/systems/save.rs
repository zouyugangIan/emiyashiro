...
pub fn save_game(
    ...
    let save_data = SaveData {
        ...
        best_distance: game_stats.distance_traveled.max(
            save_manager.current_save.as_ref()
    ...
    match serde_json::to_string_pretty(&save_data) {
        Ok(json_string) => {
            match fs::write(&save_manager.save_file_path, json_string) {
                Ok(_) => {
                    save_manager.current_save = Some(save_data);
                    println!("💾 游戏已保存！");
                    println!("   最佳距离: {:.1}m", save_manager.current_save.as_ref().unwrap().best_distance);
                    println!("   总跳跃次数: {}", save_manager.current_save.as_ref().unwrap().total_jumps);
                    println!("   总游戏时间: {:.1}s", save_manager.current_save.as_ref().unwrap().total_play_time);
                }
                Err(e) => {
                    println!("❌ 保存失败: {}", e);
                }
...
pub fn load_game(
    ...
    match fs::read_to_string(&save_manager.save_file_path) {
        Ok(json_string) => {
            match serde_json::from_str::<SaveData>(&json_string) {
                Ok(save_data) => {
                    ...
                    println!("   保存时间: {}", save_data.save_time.format("%Y-%m-%d %H:%M:%S"));
                }
                Err(e) => {
                    println!("❌ 存档文件损坏: {}", e);
                }
