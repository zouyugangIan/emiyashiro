...
/// 异步加载存档元数据
async fn load_save_metadata_async(
    save_path: PathBuf,
...
/// 压缩数据
fn compress_data(data: &[u8], compression_level: u32) -> Result<Vec<u8>, SaveSystemError> {
...
/// 解压缩数据
fn decompress_data(data: &[u8]) -> Result<Vec<u8>, SaveSystemError> {
