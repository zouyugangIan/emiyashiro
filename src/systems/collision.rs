...
pub fn collision_detection_system(
    mut player_query: Query<(&mut Transform, &mut Velocity, &mut PlayerState, &CollisionBox), With<Player>>,
    ground_query: Query<(&Transform, &CollisionBox), (With<Ground>, Without<Player>)>,
    mut commands: Commands,
) {
    if let Ok((mut player_transform, mut player_velocity, mut player_state, player_collision)) = player_query.single_mut() {
...
fn check_aabb_collision(rect1: &Rect, rect2: &Rect) -> CollisionResult {
    ...
    if overlap_x < overlap_y {
        ...
        result.contact_point = Vec2::new(
            if rect1.center().x < rect2.center().x { rect2.min.x } else { rect2.max.x },
            rect1.center().y,
        );
    } else {
        // 垂直碰撞
        ...
        result.contact_point = Vec2::new(
            rect1.center().x,
            if rect1.center().y < rect2.center().y { rect2.min.y } else { rect2.max.y },
        );
    }
    ...
    if let Ok((mut transform, mut velocity, mut player_state)) = player_query.single_mut() {
        ...
        if ground_check_result.hit {
            ...
            if ground_distance < 5.0 {
                transform.translation.y = ground_check_result.point.y + GameConfig::PLAYER_SIZE.y * 0.5;
                if velocity.y < 0.0 {
                    velocity.y = 0.0;
                }
            }
