...
- `S` 或 `↓`: 蹲下
- `J/Z/L`: 默认平A，近战挥刀
- `Y/U/I/O/P`: 1P 轻攻击表第 1-5 行
- `Shift+Y/U/I/O/P`: 1P 重攻击表第 1-5 行
- `X`: 投影攻击（远程）
- `Shift+V`: 绯红圣骸布/参考攻击模组开启，持续 10 秒并消耗 5 HP
- 绯红圣骸布期间：`J/Z/L` 打出轻攻击模组，空中轻击显示空中连段，蹲下轻击显示机动模组，`K` 使用重攻击，蹲下 `K` 使用奥义模组，`X` 显示忍术/投影模组，蹲下 `X` 显示武器投影模组
- `Esc`: 暂停
- `R`: GameOver 后复活
- `M`: GameOver 返回主菜单
